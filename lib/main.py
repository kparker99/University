...
from lib.setup_gui import Ui_MainWindow
import lib.sql_functions as sql

############      NAMING STANDARDS      ############
# Pages / Items: < l | n | t | s >_< page >_< label >_< field | btn | drop | table | label >  ex. n_firstname_field, s_classgrades_class_drop
# Functions: < t | s >_< page >_< function >  ex. _logout, s_profile_profile_data
...
class GUI_Window(qtw.QMainWindow):
    ...
    # BUTTON FUNCTIONALITY BELOW
        # page 0 (login_page)
        self.ui.l_login_btn.clicked.connect(self.login)
        self.ui.l_newaccount_btn.clicked.connect(
            lambda: self.ui.stackedWidget.setCurrentIndex(1))

        # page 1 (new_account_page)
        self.ui.n_firstname_field.textChanged.connect(self.fields_empty)
        self.ui.n_lastname_field.textChanged.connect(self.fields_empty)
        self.ui.n_address_field.textChanged.connect(self.fields_empty)
        self.ui.n_city_field.textChanged.connect(self.fields_empty)
        self.ui.n_state_field.textChanged.connect(self.fields_empty)
        self.ui.n_phone_field.textChanged.connect(self.fields_empty)
        self.ui.n_enterpassword_field.textChanged.connect(
            self.fields_empty)
        self.ui.n_confirmpassword_field.textChanged.connect(
            self.fields_empty)
        self.ui.n_createaccount_btn.clicked.connect(self.create_account)
        self.ui.n_back_btn.clicked.connect(
        ...
        self.ui.s_courseenroll_enroll_btn.clicked.connect(
            self.s_courseenroll_enroll_in_class)
        self.ui.s_grades_btn.clicked.connect(
            lambda: self.ui.stackedWidget_2.setCurrentIndex(3))
        self.ui.s_grades_btn.clicked.connect(
            self.s_classgrades_test_grade_data)
        self.ui.s_classgrades_class_drop.activated.connect(
            self.s_classgrades_test_grade_data)
        self.ui.s_logout_btn.clicked.connect(self.logout)
        self.ui.s_courseenroll_close_btn.clicked.connect(
            lambda: self.ui.stackedWidget_2.setCurrentIndex(1))
        self.ui.s_enroll_enroll_table.cellClicked.connect(
            self.s_courseenroll_class_data)

        # page 3 (teacher_page)
        self.ui.t_profile_btn.clicked.connect(self.t_profile_profile_data)
        self.ui.t_classes_btn.clicked.connect(
            lambda: self.ui.stackedWidget_3.setCurrentIndex(1))
        self.ui.t_classes_btn.clicked.connect(self.t_grades_grade_data)
        self.ui.t_logout_btn.clicked.connect(self.logout)
        self.ui.t_grades_class_drop.activated.connect(self.t_grades_grade_data)
        self.ui.t_addgrade_class_drop.activated.connect(
            self.t_get_students_add)
        self.ui.t_grades_addgrade_btn.clicked.connect(
            lambda: self.ui.stackedWidget_3.setCurrentIndex(2))
        ...
        self.ui.t_editgrade_class_drop.activated.connect(
            self.t_get_students_edit)
        self.ui.t_editgrade_name_drop.activated.connect(
            self.t_get_student_names_edit)
        self.ui.t_grades_editgrade_btn.clicked.connect(
            lambda: self.ui.stackedWidget_3.setCurrentIndex(3))
        self.ui.t_grades_editgrade_btn.clicked.connect(
            self.t_get_students_edit)
    ...
    # VALIDATORS FOR FIELDS IN NEW_ACCOUNT_PAGE HERE
        ...
        state_validator = qtg.QRegExpValidator(
            state_regex, self.ui.n_state_field)
        self.ui.n_state_field.setValidator(state_validator)

        # first name, last name, city fields
        name_regex = qtc.QRegExp("([a-zA-Z-_]+ )+")

        first_name_validator = qtg.QRegExpValidator(
            name_regex, self.ui.n_firstname_field)
        self.ui.n_firstname_field.setValidator(first_name_validator)
        last_name_validator = qtg.QRegExpValidator(
            name_regex, self.ui.n_lastname_field)
        ...
        grade_validator = qtg.QRegExpValidator(
            grade_regex, self.ui.t_addgrade_grade_field)
    ...
    def login(self):
        ...
        info = sql.get(
            "SELECT user_id, password FROM university.login WHERE user_id=%s AND password=%s", (username, password))
        if info != ():
            query = sql.get(
                "SELECT role_id, f_name, l_name FROM university.person WHERE user_id=%s", username)
            role, first, last = query[0]
            if role == 1:
                self.ui.stackedWidget.setCurrentIndex(2)
                self.ui.stackedWidget_2.setCurrentIndex(0)
                self.ui.s_userwelcome_field.setText(
                    "{} {}".format(first, last))
                ...
                self.ui.t_userwelcome_field.setText(
                    "{} {}".format(first, last))
    ...
    def create_email(self, username):
        ...
        counter = 1
        n = sql.get_one(
            "SELECT user_id FROM university.login WHERE user_id=%s", (username))
        while n != None:
    ...
    def create_account(self):
        ...
        if self.ui.n_firstname_field.text() and \
           ...
           self.ui.n_confirmpassword_field.text():
            first_name, last_name, dob, address, city, state, phone, password, confirm_password = \
                ...
                self.ui.n_phone_field.text(), self.ui.n_enterpassword_field.text(
                ), self.ui.n_confirmpassword_field.text()
        else:
        ...
        if password == confirm_password:
            ...
            # create unique email and username from create_email function
            email, username = self.create_email(possible_email)
            sql.write(
                "INSERT INTO university.login (user_id, password) VALUES (%s,%s)", (username, password))
            sql.write("INSERT INTO university.person (role_id, user_id, f_name, l_name, address, city, state, dob, email, phone) \
    ...
    def s_profile_profile_data(self):
        ...
        self.ui.stackedWidget_2.setCurrentIndex(0)
        username = self.ui.l_username_field.text()
        info = sql.get("SELECT f_name, l_name, address, city, state, dob, phone, email \
                                    FROM university.person WHERE user_id=%s", username)
        for row_number, row_data in enumerate(info):
            for column_number, data in enumerate(row_data):
                self.ui.s_profile_profile_table.setItem(
    ...
    def s_courseenroll_class_data(self, row):
        ...
        info = sql.get(
            "SELECT hours, course_description FROM university.course WHERE course_name = %s", course)
        ...
        self.ui.s_profile_profile_table.resizeColumnsToContents()
        table_info = sql.get("SELECT c.start_date, c.end_date, c.room, c.instructor, o.course_number \
    ...
    def s_courseenroll_enroll_in_class(self):
        ...
        query = sql.get_one("SELECT class_id FROM university.class \
                                          WHERE course_number=%s AND start_date=%s", (course, start))
        course_number = query[0]
        query = sql.get("SELECT role_id FROM university.enrolled WHERE user_id=%s AND class_id=%s",
                        (username, course_number))
        if query == ():
            sql.write("INSERT INTO university.enrolled (user_id, class_id, role_id) VALUES (%s,%s,1)",
                      (username, course_number))
    ...
    def s_classgrades_test_grade_data(self):
        ...
        username = self.ui.l_username_field.text()
        info = sql.get(
            "SELECT test_date, grade FROM university.grades WHERE user_id=%s AND class_id=%s", (username, course))
        if info == ():
    ...
    def t_grades_teacher_classes(self, username):
        ...
        class_list = []
        table_info = sql.get("SELECT c.class_id \
                                          FROM university.class c \
    ...
    def t_grades_grade_data(self):
        ...
        info = sql.get(
            "SELECT user_id, test_number, test_date, grade FROM university.grades WHERE class_id=%s", (course))
        if info == ():
    ...
    def t_addgrade(self):
        ...
        if query == ():
            sql.write("INSERT INTO university.grades (class_id, user_id, test_number, test_date, grade) VALUES (%s,%s,%s,%s,%s)",
                      (course, name, test, date, grade))
            self.message("Grade Added", "Test grade added")
        else:
            self.message(
                "Error", "Student already has grade entered for test number")
